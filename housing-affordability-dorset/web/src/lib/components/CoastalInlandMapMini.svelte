<script lang="ts">
	...
	onMount(async () => {
		...
		for (const feature of geojson.features) {
			...
			feature.properties = { ...feature.properties, coastalInland: group ?? '' };
		}
	...
	$effect(() => {
		...
		if (!layer) return;
		mapInstance.setPaintProperty('msoa-fill', 'fill-opacity', mapOpacity);
	});
</script>
...
<div
	...
	role="img"
	aria-label="Small reference map showing coastal and inland MSOAs across Dorset. Coastal areas are teal and inland areas are grey."
>
